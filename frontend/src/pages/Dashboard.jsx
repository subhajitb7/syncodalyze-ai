...
import { FileCode, History, Plus, AlertCircle, CheckCircle2, BarChart3, Bug, CheckCircle } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const getSnippet = (text) => {
    ...
    let cleanText = text.replace(/^#+\s*(Code Review|AI Code Review).*\n/i, '');
    cleanText = cleanText.replace(/^(Code Review|AI Code Review)\s*/i, '');
    cleanText = cleanText.replace(/[#*_~`>]/g, '');
    cleanText = cleanText.replace(/^(Overview:?|Here is an overview:?)\s*/i, '');
    cleanText = cleanText.replace(/\[(.*?)\]\(.*?\)/g, '$1');
    cleanText = cleanText.replace(/\s+/g, ' ');
    return cleanText.trim().substring(0, 100) + '...';
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredReviews.map((review) => (
              <Link
                ...
                className="bg-dark-900/50 border border-dark-600 rounded-xl p-5 hover:border-primary-500/50 transition-all flex flex-col h-48 group"
              >
