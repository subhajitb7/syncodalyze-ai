  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
      <div className="mb-8">
        ...
        <p className="text-gray-400 mt-1">Manage the platform, users, and content.</p>
      </div>
      ...
      <div className="flex gap-2 mb-8">
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === t.key ? 'bg-primary-500/10 text-primary-400 border border-primary-500/30' : 'text-gray-400 border border-dark-600 hover:border-gray-500'}`}>
                ...
                <div className="space-y-2">
                  {stats.recentUsers.map((u) => (
                    <div key={u._id} className="flex items-center justify-between p-3 bg-dark-900/50 border border-dark-600 rounded-lg">
                      ...
                      <span className={`text-xs px-2 py-1 rounded ${u.role === 'admin' ? 'bg-primary-500/20 text-primary-400' : 'bg-dark-700 text-gray-400'}`}>{u.role}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-dark-600 text-left">
                      ...
                      <th className="p-4 text-gray-400 font-medium">Verified</th>
                  ...
                  <tbody>
                    {users.map((u) => (
                      <tr key={u._id} className="border-b border-dark-700/50 hover:bg-dark-800/50">
                        ...
                        <td className="p-4">
                          <span className={`text-xs ${u.isVerified ? 'text-emerald-400' : 'text-yellow-400'}`}>{u.isVerified ? 'Yes' : 'No'}</span>
                        </td>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-dark-600 text-left">
                      ...
                      <th className="p-4 text-gray-400 font-medium">Lang</th>
                      <th className="p-4 text-gray-400 font-medium">Bugs</th>
                  ...
                  <tbody>
                    {reviews.map((r) => (
                      <tr key={r._id} className="border-b border-dark-700/50 hover:bg-dark-800/50">
                        <td className="p-4 font-medium">{r.title}</td>
                        <td className="p-4 text-gray-400">{r.user?.name || 'Unknown'}</td>
                        <td className="p-4 text-primary-400 uppercase text-xs">{r.language}</td>
                        <td className="p-4">
                          <span className={r.bugsFound > 0 ? 'text-red-400' : 'text-emerald-400'}>{r.bugsFound}</span>
                        </td>
